...
def queue_subtasks_for_query(
    ...
    # Take into account outsidequeryset items
    sup_subtask_outside = 0
    items_outside_count = 0
    if outside_queryset_additional_items:
        sup_subtask_outside = 1
        items_outside_count = len(outside_queryset_additional_items)

    total_num_subtasks = _get_number_of_subtasks(total_num_items, items_per_task)
    subtask_id_list = [str(uuid4()) for _ in range(total_num_subtasks + sup_subtask_outside)]
    ...
    # On purpose we do not add the outsidequeryset items count to the total_num_subtasks and remove also from count
    ...
    # Deal with additionnal subtask for outside query items
    if outside_queryset_additional_items is not None:
        TASK_LOG.info(
            "Task %s: creating subtasks to process outside query items.",
            task_id
        )
        ...
        num_subtasks += 1
        subtask_status = SubtaskStatus.create(subtask_id)
        new_subtask = create_subtask_fcn(outside_queryset_additional_items, subtask_status)
        new_subtask.apply_async()
